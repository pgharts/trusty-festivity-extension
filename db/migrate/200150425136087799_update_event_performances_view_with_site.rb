class UpdateEventPerformancesViewWithSite < ActiveRecord::Migration
  def up
    execute <<-SQL
      CREATE OR REPLACE VIEW festivity_event_performances AS
        SELECT
          ...
          event.featured_item, event.header, event.sub_header, event.site_id, location.id as location_id,
